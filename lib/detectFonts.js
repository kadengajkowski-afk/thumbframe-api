...
// Sonnet 4.6 alias — matches the existing model ID at index.js:3513.
// Day 33 originally shipped with a guessed dated suffix
// ('claude-sonnet-4-6-20250929') which Anthropic 404'd on, the SDK
// threw, the outer try/catch returned [] silently. Cached payloads
// across two channels confirmed empty fonts.
 ...
 *
 * All failure modes log to Railway and return []. Logs are tagged
 * [BRAND-KIT/fonts] so they're greppable. */
async function detectFonts({ anthropic, thumbnailUrls }) {
  if (!anthropic) {
    console.log('[BRAND-KIT/fonts] anthropic client missing — skipping detection');
    return [];
  }
  if (!Array.isArray(thumbnailUrls) || thumbnailUrls.length === 0) {
    console.log('[BRAND-KIT/fonts] no thumbnails — skipping detection');
    return [];
  }

  const urls = thumbnailUrls.slice(0, MAX_THUMBS);
  console.log(`[BRAND-KIT/fonts] starting detection on ${urls.length} thumbs (model=${VISION_MODEL})`);
  ...
  if (validImages.length === 0) {
    console.warn('[BRAND-KIT/fonts] all image fetches failed — returning []');
    return [];
  }
  ...
  try {
    ...
    const filtered = filterFonts(parsed.fonts);
    console.log(
      `[BRAND-KIT/fonts] detected ${filtered.length} font(s) from ${Array.isArray(parsed.fonts) ? parsed.fonts.length : 0} candidate(s):`,
      filtered.map((f) => `${f.name} (${f.confidence.toFixed(2)})`).join(', ') || '(none above floor)',
    );
    return filtered;
  } catch (err) {
    // SDK errors carry .status (HTTP) + .error.type. Surface both so a
    // 404 on a bad model id, 429 on rate limit, or 401 on bad key is
    // immediately diagnosable from Railway logs.
    const status = err?.status || err?.response?.status || '?';
    const type   = err?.error?.type || err?.name || '?';
    console.warn(
      `[BRAND-KIT/fonts] vision call failed (status=${status} type=${type}):`,
      err.message,
    );
    return [];
  }
